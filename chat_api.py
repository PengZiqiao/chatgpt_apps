...
def slice_prompts(prompts):
    ...
    for message in reversed(prompts):
        ...
        if total_length > 2000:
            break
    # 返回 prompts 列表中从 start_index-1 到末尾的子列表作为结果
    return prompts[start_index-1:]
...
class SessionChat:
    ...
    def __call__(self, prompt):
        ...
        response = openai.ChatCompletion.create(model="gpt-3.5-turbo",
                                                messages=[self.system_massage] + self.prompts,
...
class Chat:
    ...
    def __call__(self, prompts, temperature=0, system_message=None):
